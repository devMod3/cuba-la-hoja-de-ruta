...
export class AboutFeature {
  ...
  render(data) {
    ...
    const hasProfile = Boolean(profile.displayName || profile.photoUrl || profile.introduction || profile.occupation || profile.industry || profile.gender || joinedLocation(profile.location) || profile.email || profile.website || profile.bloggerProfileUrl || profile.interests?.length || profile.favoriteMovies?.length || profile.favoriteMusic?.length || profile.favoriteBooks?.length);
    ...
    if (hasProfile) {
      ...
      appendDefinition(defs, 'Introducción', profile.introduction);
    ...
    if (social.length) {
      ...
      social.sort((a, b) => a.order - b.order).forEach((item) => {
        ...
        if (!a) return;
        a.replaceChildren(
          node('span', 'zen-about-social-name', item.label || SOCIAL_LABELS.get(item.platform) || 'Red social'),
          node('span', 'zen-about-social-user', item.username || 'Abrir ↗')
        );
        list.appendChild(a);
      });
